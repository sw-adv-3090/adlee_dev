...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $data['tasks'] = Task::with(['coupon:id,uuid,title,language,payout_on', 'template:id,preview', 'sponsor'])->where('assign_to', auth()->id())->whereNotNull(['template_id', 'printed_at'])->whereHas('coupon', fn($q) => $q->whereNull('payout_at'))->latest()->get();
