            ...
            <div class="card">
                <div class="tabs flex flex-col">
                    <div class="tab-content p-4 sm:p-5">
                        <form action="{{ route('admin.templates.sponsors.update', $template) }}" method="post"
                            ...
                            <div class="space-y-5">
                                ...
                                <label class="block">
                                    ...
                                    <select
                                        ...
                                        @foreach ($languages as $language)
                                            <option value="{{ $language->value }}" @selected($language->value === $template->language)>
                                ...
                                @livewire('template-category', ['category_id' => $template->category_id, 'sub_category_id' => $template->sub_category_id])
                                ...
                                <div>
                                    ...
                                    <p class="my-1.5">Upload image of template who it will look. This will help users to
                                        ...
                                        If you want to use old preview image then don't upload new image. leave it
                                        blank.
                                    </p>
                                    <div class="filepond fp-bordered fp-grid mt-1.5 [--fp-grid:2] ">
                                        <input type="file" name="preview" class="preview" />
                                    </div>
                                    @error('preview')
                                        <span class="text-xs text-error">{{ $message }}</span>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    @include('partials.sponsor-tooltip')
                                    <div class="filepond fp-bordered fp-grid mt-1.5 [--fp-grid:2]">
                                        <input type="file" name="content" class="content" />
                                    </div>
                                    @error('content')
                                        <span class="text-xs text-error">{{ $message }}</span>
                                    @enderror
                                ...
                                <label class="block">
                                    ...
                                    <p class="my-1.5">The date at which the template will be visible for sponsors to
                                    ...
                                    <span class="relative mt-1.5 flex">
                                        <input x-init="$el._x_flatpickr = flatpickr($el, {
                                            defaultDate: '{{ old('publish_at', $template->publish_at->format('Y-m-d')) }}',
                                            altInput: true,
                                            altFormat: 'F j, Y',
                                            dateFormat: 'Y-m-d',
                                        })"
